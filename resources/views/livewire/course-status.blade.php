    ...
    <div class="container grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="flex justify-between mt-4">
                ...
                <div class="flex items-center cursor-pointer" wire:click="download">
                    ...
                    <p class="text-gray-600 text-sm ml-2 ">Descargar recurso</p>
                </div>
                @endif
            </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="text-gray-600 text-small mt-2">{{ $this->advance }}% completado</p>

                <div class="relative pt-1">
                    <div class="overflow-hidden h-2 text-xs flex rounded bg-green-200">
                        <div style="width: {{ $this->advance }}%" class="
                            shadow-none
                            flex flex-col
                            text-center
                            whitespace-nowrap
                                ...
                                <div>
                                    @if($lesson->completed)
                                        @if($current->id == $lesson->id)
                                            <span class="inline-block w-4 h-4 border-2 border-yellow-500 rounded-full mt-1"></span>
                                        @else
                                            <span class="inline-block w-4 h-4 bg-yellow-500 rounded-full mt-1"></span>
                                        @endif
                                    @else
                                        @if($current->id == $lesson->id)
                                            <span class="inline-block w-4 h-4 border-2 border-gray-500 rounded-full mt-1"></span>
                                        @else
                                            <span class="inline-block w-4 h-4 bg-gray-500 rounded-full mt-1"></span>
                                        @endif
                                    @endif
                                </div>
